...
# <a name="assign-permissions-to-users-and-groups"></a><a name="assign-permissions-to-users-and-groups"></a><a name="assign-permissions-to-users-and-groups"></a>Úthluta leyfum til notenda og hópa
...
Þessi grein lýsir hvernig á að skilgreina, nota og nota heimildir til [!INCLUDE [prod_short](includes/prod_short.md)] að breyta sjálfgefnum stillingum.  
...
[!INCLUDE [prod_short](includes/prod_short.md)] á netinu felur í sér sjálfgefna notendahópa sem eru úthlutað til notenda sjálfkrafa byggt á heimildum þeirra. Hægt er að breyta sjálfgefinni skilgreiningu með því að breyta eða bæta við öryggisflokkum, heimildasamstæðum og heimildum. Eftirfarandi tafla sýnir helstu sviðsmyndir til að breyta sjálfgefnum heimildum.  

|Til  |Sjá  |
|---------|---------|
|Til að auðvelda stjórnun heimilda fyrir marga notendur er hægt að raða þeim í öryggisflokka og síðan úthluta eða breyta einni heimildasamstæðu fyrir marga notendur í einni aðgerð.| [Að stjórna heimildum í gegnum notendaflokka](#to-manage-permissions-through-user-groups) |
...
## <a name="to-create-a-permission-set"></a><a name="to-create-a-permission-set"></a><a name="to-create-a-permission-set"></a>Til að stofna heimildasamstæðu
...
Einnig er auðveldara að sinna viðhaldi. Þegar kerfisheimild er bætt við mun notendaskilgreind heimildasamstæða þín verða sjálfkrafa uppfærð með öllum breytingum sem Microsoft gerir á þessum heimildum.
...
5. Á síðunni **Heimildasamstæða** í reitnum **Tegund** skal setja inn eða útiloka heimildir fyrir hlutinn sem hér segir:
  ...
  > [!NOTE]
  > Ef heimild er í heimildasamstæðu sem er innifalin, og er einnig í heimildasamstæðu sem er undanskilin, verður heimildin undanskilin.
...
### <a name="example---indirect-permission"></a><a name="example---indirect-permission"></a><a name="example---indirect-permission"></a>Dæmi - Óbein heimild
...
### <a name="to-add-other-permission-sets"></a><a name="to-add-other-permission-sets"></a><a name="to-add-other-permission-sets"></a>Til að bæta við öðrum heimildasettum
...
Til að útiloka allar heimildir að fullu frá heimildasamstæðu **skal velja línuna á rúðunni**, velja Sýna fleiri valkosti **og velja** **svo Útiloka**. Þegar þú undanskilur heimildasamstæðu er búin til lína í glugganum **Heimildasamstæður** af gerðinni Útilokað. Ef þú hefur útilokað heimildasamstæðu en vilt láta það fylgja með aftur, skaltu eyða línunni í glugganum **Heimildasamstæður**.

Til að útiloka að fullu eða að hluta tiltekna heimildir í samstæðu sem þú hefur bætt við skaltu búa til línu fyrir hlutinn fyrir neðan **Heimildir**. Aðgangsstigssvæðin, Setja inn heimild, Breyta heimild, og svo framvegis, munu öll innihalda **Útiloka**. Veljið viðeigandi valkost til að leyfa tiltekið aðgangsstig.

Að undanskildum heimildasamstæðu útilokar allar heimildir í safninu. [!INCLUDE [prod_short](includes/prod_short.md)] reiknar heimildir á eftirfarandi hátt:

1. Reiknaðu allan listann yfir meðfylgjandi heimildir
2. Reiknaðu allan listann yfir útilokaðar heimildir
3. Fjarlægja útilokaðar heimildir af listanum yfir meðfylgjandi heimildir (að fjarlægja óbeina heimild er það sama og Minnka í óbeint)

## <a name="to-copy-a-permission-set"></a><a name="to-copy-a-permission-set"></a><a name="to-copy-a-permission-set"></a>Til að afrita heimildasamstæðu
...
## <a name="to-create-or-modify-permissions-by-recording-your-actions"></a><a name="to-create-or-modify-permissions-by-recording-your-actions"></a><a name="to-create-or-modify-permissions-by-recording-your-actions"></a>Að stofna eða breyta heimildum með skráning við aðgerðir þínar
...
5. Á síðunni **Heimildir** skal velja aðgerðina **Skrá heimildir** og velja svo aðgerðina **Byrja**.  
    Skráning verður að fara fram annaðhvort með því að nota **Opna þessa síðu í nýjum gluggum**  (pop-out) til að hafa **heimildarupptökugluggann** hlið við hlið, eða með því að vinna innan sama flipa.  
    Upptökuferli hefst nú og fangar allar aðgerðir þínar í notendaviðmótinu.
...
### <a name="to-export-and-import-a-permission-set"></a><a name="to-export-and-import-a-permission-set"></a><a name="to-export-and-import-a-permission-set"></a>Til að flytja út og flytja inn heimildasamstæðu
...
## <a name="to-remove-obsolete-permissions-from-all-permission-sets"></a><a name="to-remove-obsolete-permissions-from-all-permission-sets"></a><a name="to-remove-obsolete-permissions-from-all-permission-sets"></a>Til að fjarlægja úreltar heimildir úr öllum heimildasamstæðum

Á síðunni **Heimildasamstæður** skal velja aðgerðina **Fjarlægja úreltar heimildir**.

## <a name="to-set-up-time-constraints-for-users"></a><a name="to-set-up-time-constraints-for-users"></a><a name="to-set-up-time-constraints-for-users"></a>Uppsetning tímatakmarkana fyrir notendur
...
## <a name="to-manage-permissions-through-user-groups"></a><a name="to-manage-permissions-through-user-groups"></a><a name="to-manage-permissions-through-user-groups"></a>Að stjórna heimildum í gegnum notendaflokka
...
### <a name="to-add-users-to-a-user-group"></a><a name="to-add-users-to-a-user-group"></a><a name="to-add-users-to-a-user-group"></a>Til að bæta notendum í notendahóp
...
### <a name="to-copy-a-user-group-and-all-its-permission-sets"></a><a name="to-copy-a-user-group-and-all-its-permission-sets"></a><a name="to-copy-a-user-group-and-all-its-permission-sets"></a>Til að afrita notendaflokk og öll heimildarsöfn
...
### <a name="to-assign-permission-sets-to-user-groups"></a><a name="to-assign-permission-sets-to-user-groups"></a><a name="to-assign-permission-sets-to-user-groups"></a>Til að úthluta leyfishópum á notendahópa
...
## <a name="to-assign-permission-sets-to-users"></a><a name="to-assign-permission-sets-to-users"></a><a name="to-assign-permission-sets-to-users"></a>Til að úthluta heimildasamstæðu á notendur
...
### <a name="to-assign-a-permission-set-on-a-user-card"></a><a name="to-assign-a-permission-set-on-a-user-card"></a><a name="to-assign-a-permission-set-on-a-user-card"></a>Til að úthluta heimildasamstæðu á notendakorti
...
### <a name="to-assign-a-permission-set-on-the-permission-set-by-user-page"></a><a name="to-assign-a-permission-set-on-the-permission-set-by-user-page"></a><a name="to-assign-a-permission-set-on-the-permission-set-by-user-page"></a>Til að úthluta heimildasamstæðu á síðunni Heimildasamstæða eftir notanda
...
## <a name="to-get-an-overview-of-a-users-permissions"></a><a name="to-get-an-overview-of-a-users-permissions"></a><a name="to-get-an-overview-of-a-users-permissions"></a>Fá yfirlit yfir heimildir notanda
...
### <a name="security-filters-limit-a-users-access-to-specific-records-in-a-table"></a><a name="security-filters-limit-a-users-access-to-specific-records-in-a-table"></a><a name="security-filters-limit-a-users-access-to-specific-records-in-a-table"></a>Öryggissíur takmarka aðgang notanda að tilteknum færslum í töflu
...
## <a name="viewing-permission-changes-telemetry"></a><a name="viewing-permission-changes-telemetry"></a><a name="viewing-permission-changes-telemetry"></a>Skoða fjarmælingar heimildabreytinga
...
## <a name="delegated-admin-users"></a><a name="delegated-admin-users"></a><a name="delegated-admin-users"></a>Úthlutaðir stjórnendur
