...
> 
> Það verður að stofna jöfnunarskráningu í Azure gáttina og keyra síðan Uppsetningarleiðbeiningar fyrir  **Microsoft Entra  aðstoðarkenni**  í  [!INCLUDE[prod_short](includes/prod_short.md)]  til að tengjast  Microsoft Entra  Kenni. Frekari upplýsingar er að finna í [Búa til forritsskráningu fyrir Business Central í Azure-gátt](admin-how-setup-email.md#create-an-app-registration-for-business-central-in-azure-portal).
>
...
## <a name="use-the-set-up-email-assisted-setup-guide"></a>Nota leiðbeiningar um uppsetningu tölvupósts með aðstoð
...
is this still true?-->
## <a name="assign-email-scenarios-to-email-accounts"></a>Tengja tölvupóstaðstæður við tölvupóstreikninga
...
## <a name="specify-how-many-messages-an-account-can-send-per-minute"></a>Tilgreina hversu mörg boð geta sent reikning á mínútu
...
## <a name="set-up-document-sending-profiles"></a>Setja upp forstillingar fyrir sendingu skjals
...
## <a name="optional-monitor-email-usage-and-troubleshoot-email-failures-with-telemetry"></a>Valfrjálst: fylgjast með tölvupóstnotkun og leysa úr mistökum í tölvupósti með Telemetry

Kerfisstjórar geta kveikt á eiginleikum sjónins í  [!INCLUDE[prod_short](includes/prod_short.md)]  til að fá gögn um notkun og bilanir af mismunandi getu í kerfinu. Í tölvupósti skráir þú eftirfarandi aðgerðir:

- Sent var með tölvupósti  
- Tilraun til að senda tölvupóst mistókst   
- Sannvottun á SMTP-þjóni tókst/mistókst  
- Tenging við SMTP þjón tókst/mistókst  

Hægt er að nota þessi gögn til að fylgjast með tölvupóstnotkun og til að leita að mistökum. Frekari upplýsingar í  [greiningu netfang Telemetry (stjórnunarefni)](/dynamics365/business-central/dev-itpro/administration/telemetry-email-trace).  

## <a name="set-up-email-for-business-central-on-premises"></a>Setja upp email fyrir Business Mið innanhúss

[!INCLUDE[prod_short](includes/prod_short.md)] á staðnum er hægt að samþætta við þjónustur sem byggja á Microsoft Azure. Til dæmis er hægt að nota Cortana Intelligence fyrir snjallari sjóðsstreymisspá, Power BI til að sjá fyrir sér reksturinn og Exchange Online til að senda tölvupóst. Samþætting við þessa þjónustu byggist á App skráningu í  Microsoft Entra  auðkenni. Forritsskráningin býður upp á sannvottun og heimildaþjónusta fyrir samskipti. Til að nota möguleika tölvupóstsins í [!INCLUDE[prod_short](includes/prod_short.md)] þarf að skrá [!INCLUDE[prod_short](includes/prod_short.md)] sem forrit í Azure-gáttinni og síðan tengja [!INCLUDE[prod_short](includes/prod_short.md)] við forritsskráninguna. Eftirfarandi hlutar útskýra hvernig.
...
Leiðbeiningum til skráningar  [!INCLUDE[prod_short](includes/prod_short.md)]  í Azure Portal er lýst með  [því að skrá umsókn í  Microsoft Entra  ID](/dynamics365/business-central/dev-itpro/administration/register-app-azure#register-an-application-in-azure-active-directory).
...
Frekari upplýsingar um almennar leiðbeiningar fyrir skráningu í App hjá  [Quickstart: Skráðu þig inn umsókn með Platform Microsoft auðkenni](/azure/active-directory/develop/quickstart-register-app).
...
### <a name="connect--to-your-app-registration"></a>Tengjast  [!INCLUDE[prod_short](includes/prod_short.md)]  við App skráningu þína

Eftir að þú hefur skráð þína umsókn í Azure Portal, í  [!INCLUDE[prod_short](includes/prod_short.md)], Notaðu  **Kennisíðu  Microsoft Entra  tölvupóstforritsins**  til að tengjast  [!INCLUDE[prod_short](includes/prod_short.md)]  því.

1. Í [!INCLUDE[prod_short](includes/prod_short.md)] skal velja ![Ljósapera sem opnar eiginleika Viðmótsleitar.](media/ui-search/search_small.png "Segðu mér hvað þú vilt gera") Táknið, Sláðu inn  **Kenniskráningu  Microsoft Entra  umsóknar** og veldu síðan tengda tengilinn.
2. Fyllið inn reitina eftir þörfum. [!INCLUDE[tooltip-inline-tip](includes/tooltip-inline-tip_md.md)]

> [!TIP]
> Annars, ef verið er að tengjast í fyrsta skipti, er hægt að keyra uppsetningarleiðbeininguna **Setja upp tölvupóst** með hjálp. Í því tilviki skal handbókin einnig innihalda skráningarsíðu kennis tölvupósts  Microsoft Entra  til að bæta við upplýsingum fyrir tengingu við App skráningu. <!--Need to verify this too. Ask John to clear the aad settings, delete the email accounts, and then run the guide.-->
...
[Að komast  [!INCLUDE[prod_short](includes/prod_short.md)]  á Farsímatækið mitt](install-mobile-app.md)   
[Að komast  [!INCLUDE[prod_short](includes/prod_short.md)]  á Farsímatækið mitt](install-mobile-app.md)   
[Greining Pósttelemetry (innihald stjórnunar)](/dynamics365/business-central/dev-itpro/administration/telemetry-email-trace)  
