...
Þetta efnisatriði lýsir skýrslunum í [!INCLUDE[prod_short](includes/prod_short.md)] sem þú getur notað til að senda inn upplýsingar um virðisaukaskatt (VSK) upphæðir fyrir sölu og innkaup til skattayfirvalda á þínu svæði. Skýrslurnar geta innihaldið tilteknar upplýsingar eða frekari skýrslur sem þarf að senda til að fara eftir tilteknu landi/svæði. Skoðaðu greinar fyrir þitt land/svæði í  [hlutanum staðbundin virkni](about-localization.md) .  

Hægt er að nota eftirfarandi innbyggðar skýrslur:

* Skýrslan **EB-söluyfirlit**  

    Í skýrslu um Sölulistaskýrslur Evrópubandalagsins (EC) er birtur Virðisauki (VAT) sem safnað hefur verið fyrir sölu til VSK-skráðra viðskiptamanna í löndum Evrópusambandsins/regioins.  
* Skýrslan **VSK-skil**  

    VSK-Vöruskilaskýrslan inniheldur VSK fyrir sölu og innkaup til viðskiptamanna og lánardrottna í öllum löndum/svæðum sem nota VSK.  
...
> [!NOTE]
> Hvert  [!INCLUDE[prod_short](includes/prod_short.md)]  umhverfi er ætlað að sjá um skýrslugerð í einum landshluta. T.d. Hollenska útgáfan af sér um  [!INCLUDE[prod_short](includes/prod_short.md)]  VSK-skýrslur í Hollandi en ekki í öðrum löndum/svæðum. Á sama hátt er Bandaríkin útgáfa af  [!INCLUDE[prod_short](includes/prod_short.md)]  handföngum 1099 skýrslugerð í Bandaríkjunum og styður ekki SEGJUM VSK-skýrslugerð í öðrum löndum/svæðum nema með framlengingu sem afhent eru af vistkerfisnálgun okkar, eða með breytingu á númeri viðskiptavinar.

## <a name="ecsaleslist"></a>Um EC söluyfirlitsskýrslu

Í Evrópusambandinu (ESB) og í BRETLANDI hafa öll fyrirtæki sem selja vörur og þjónustu til VSK-skráðra viðskiptamanna, þar á meðal viðskiptavina í öðrum aðildarríkjum Evrópusambandsins (ESB), þurfa að skila inn rafrænni útgáfu af sölulistum Evrópubandalagsins (EC) til tollayfirvalda og skattyfirvalda.  **Skýrsla EC-sölulistans**  virkar eingöngu fyrir lönd/svæði innan ESB.
...
## <a name="vatreturn"></a>Um skýrsluna um VSK skil
...
## Að setja upp VSK-skýrslur í [!INCLUDE[prod_short](includes/prod_short.md)]
...
Í sumum löndum/svæðum er hægt að skiptast á boðum við skattayfirvöld þegar skýrslum er skilað. Hægt er að skoða fyrstu og síðustu skilaboðin sem voru send eða móttekin með því að velja aðgerðirnar **Hala niður innsendingarskilaboðum** og **Hala niður svarskilaboðum**.  
