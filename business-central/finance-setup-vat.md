...
# <a name="set-up-calculations-and-posting-methods-for-value-added-tax"></a>Setja upp útreikninga og bókunaraðferðir fyrir virðisaukaskatt
...
### <a name="enable-or-disable-the-vat-date-feature"></a>Gera aðgerðina VSK-dagsetning virka eða óvirka
...
> [!NOTE]
> Stýringin breytist frá og með Business Central útgáfu 23.1. Í fyrri útgáfum var aðeins ein stýring á **fjárhagur uppsetningarsíðunni** fyrir bæði Bókunardags. og VSK-dagsetningu. Nú er þessum stýringum skipt, svo að stjórn á síðunni **fjárhagur Uppsetning** er aðeins fyrir **bókunardagsetninguna** og stýring á síðunni **VSK-uppsetning** er aðeins fyrir **VSK-dagsetninguna** . Einnig eru nýjar dagsetningarstýringar á síðunni **Notandauppsetning** .  

##### <a name="version-231-or-newer"></a>Útgáfa 23.1 eða nýrri

> [!IMPORTANT]
> Þegar uppfært er í nýja umbreytingu skal hafa í huga að gildi eru uppfærð í nýrri **Dags. VSK-dags. frá/til** á **síðunni VSK-uppsetning** sem byggjast á gildunum í **Bókun leyfð frá/til** í **fjárhagur uppsetningu**. Ef nota á mismunandi dagsetningarstýringar er síðan VSK-uppsetning **opnuð** og breytingar gerðar.  
...
1.  ![Veldu Lightbulb sem opnar Tell Me aðgerð 1.](media/ui-search/search_small.png "Segðu mér hvað þú vilt gera") Táknmynd, slá inn **VSK-grunn** og velja síðan tengda tengja.  
...
1.  ![Veldu Lightbulb sem opnar Tell Me aðgerð 1.](media/ui-search/search_small.png "Segðu mér hvað þú vilt gera") Táknmynd, slá inn **Notandauppsetning** og velja síðan viðeigandi tengja.  
...
## <a name="set-up-vat-registration-numbers-for-your-countryregion"></a>Setja upp VSK-númer fyrir landið/svæðið
...
Gott hugmynd að nota kóta sem auðvelt er að muna og lýsa hlutfallinu, t.d **. ÁN-VSK** eða **Núll**, **VSK10** eða **Lækkað** fyrir 10 prósent VSK og **VSK25** eða **Staðlað** fyrir 25 prósent.
...
3.  **Velja skal VSK viðsk.bókunarflokk**.  
...
2. Framkvæmdu eitt af eftirfarandi skrefum:  
    ...
    * Á birgðaspjaldinu **er flýtiflipinn** Verð & bókun **stækkaður og Sýna meira** valið **til að birta reitinn** VSK vörubókunarflokkur **.**   
    * Á **Tilfang** spjaldinu, stækkið **Reikningagerð** flipann.  
3. Veljið VSK-vörubókunarflokk.  

## <a name="set-up-clauses-to-explain-vat-exemption-or-nonstandard-vat-rates"></a>Setja upp klausur til að útskýra VSK-undanþágu eða óafgreiðanlega VSK-taxta

VSK-klausa er sett upp til að lýsa upplýsingum um hvaða tegund VSK er notuð. Stjórnvöld geta krafist þessara upplýsinga. Þegar búið er að setja upp VSK-klausa og tengja hann við VSK-bókunargrunn birtist VSK-klausa á prentuðum söluskjölum sem nota uppsetningu VSK-bókunarflokksins.

Ef með þarf er einnig hægt að tilgreina hvernig skal þýða VSK ákvæði yfir á öðrum tungumálum. Þegar söluskjal með VSK-kenni er stofnað og prentað er fylgiskjalið með þýddu VSK-klausa. Tilgreindur tungumálakóðinn á viðskiptamannaspjaldinu ákvarðar tungumálið.

Þegar engin VSK-verð eru notuð í mismunandi tegundum fylgiskjala, svo sem reikningum eða kreditreikningum, gæti þurft að taka undanþágutexta með (VSK-klausa). Undanþágutextinn segir til um hvers vegna reiknuð var lækkuð VSK eða VSK-hlutfall núll. Hægt er að skilgreina mismunandi VSK-klausur til að hafa með á viðskiptaskjölum fyrir hverja tegund fylgiskjals á síðunni **VSK-klausur eftir tegund** fylgiskjals.

Hægt er að breyta eða eyða VSK-klausa og breytingarnar birtast í skýrslu sem er búin til. Hins vegar [!INCLUDE[prod_short](includes/prod_short.md)]  heldur ekki sögu breytinganna. Í skýrslunni eru VSK-klausulýsingar prentaðar og birtar fyrir allar línur í skýrslunni ásamt VSK-upphæðinni og upphæð VSK-stofnsins. Ef VSK-klausa er ekki skilgreind fyrir neinar línur í söluskjalinu er öllum hlutanum sleppt þegar skýrslan er prentuð.
...
2. Í dálknum **VSK-klausa Kóti** skal velja ákvæðið sem nota á fyrir hverja VSK-bókunargrunn sem það á við.  
...
## <a name="use-reverse-charge-vat-for-trade-between-eu-countriesregions"></a>Nota bakfærðan VSK fyrir viðskipti milli ESB-landa/svæða
...
### <a name="sales-to-eu-countriesregions"></a>Sala til ESB-landa/svæða
...
Upphæðir í fylgiskjölum sem ekki eru bókaðar eru sléttaðar og birtar til samræmis við lokaupphæðir sléttun bókaðra upphæða. [!INCLUDE [prod_short](includes/prod_short.md)] reiknar VSK fyrir allt fylgiskjalið. VSK-útreikningurinn byggist á samtölu allra lína með sama VSK-kenni í fylgiskjalinu.  
...
[Setja upp áætlaðan virðisaukaskatt](finance-setup-unrealized-vat.md)    
...
[Staðfesta VSK-númer](finance-how-validate-vat-registration-number.md)    
[Staðbundnar aðgerðir í Business Central](about-localization.md)    
...
[Uppsetning rafrænna VSK og ICP-skýrslna í hollensku útgáfunni](LocalFunctionality/Netherlands/how-to-set-up-electronic-vat-and-icp-declarations.md)    
[VSK-skýrslur í spænsku útgáfunni](LocalFunctionality/Spain/vat-reports.md)    
[Setja upp bókun vöru og þjónustuskatts í ástralsku útgáfunni](LocalFunctionality/Australia/how-to-set-up-goods-and-service-tax-posting.md)    
[VSK í tékkneskri útgáfu](LocalFunctionality/Czech/finance-vat.md)    
