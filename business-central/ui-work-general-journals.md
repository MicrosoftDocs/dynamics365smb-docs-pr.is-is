...
Flestar færslubækur eru byggðar á færslubókinni og hægt er að vinna allar færslur á síðunni **Færslubækur** . Frekari upplýsingar er að finna á [Bóka færslu beint í Fjárhag](finance-how-post-transactions-directly.md).  
...
Hins vegar [!INCLUDE [prod_short](includes/prod_short.md)]  býðst einnig færslubókum sem eru bjartsýni fyrir tilteknar tegundir færslna, svo sem *Útgreiðslubók til* að skrá greiðslur. Nánari upplýsingar um [skráningu greiðslna og endurgreiðslna í útgreiðslubókina](payables-how-post-payments-refunds.md).  
...
Til eru nokkur færslubókarsniðmát. Hvert bókarsniðmát er táknað með sérstakri síðu með ákveðnum aðgerðum og reitum sem þurfa til að styðja þessar aðgerðir, svo sem **síðuna Greiðsluafstemmingarbækur** til að vinna bankagreiðslur og **síðuna Greiðslubækur** til að greiða lánardrottnum eða endurgreiða starfsmönnum. Fræðast meira um [greiðslur](payables-make-payments.md) og [stemma af greiðslur viðskiptamanna við inngreiðslubók eða úr viðskiptamannafærslum](receivables-how-apply-sales-transactions-manually.md).
...
Á síðunni **Ítrekunarfærslubækur** er hægt að velja aðgerðina **Úthlutun** til að tilgreina hvernig á að úthluta upphæðum í ítrekunarbókarlínu á nokkra reikninga og víddir. Úthlutun virkar sem mótreikningslína fyrir ítrekunarbókarlínuna.
...
> [!NOTE]
> Ekki er hægt að nota úthlutunarreikninga í bókarkeyrslum sem hafa samþykktarverkflæði til að stofna færslubókarlínur.
...
Eftirfarandi ferli er byggt á síðunni **Færslubækur**, en á við um allar aðrar færslubækur sem byggðar eru á færslubókinni, svo sem síðuna **Útgreiðslubækur** .
