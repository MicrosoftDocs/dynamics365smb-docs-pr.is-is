---
title: Tína fyrir innri starfsemi með ítarlegum vöruhúsaskilgreiningum
description: Ef birgðageymslur eru notaðar við tiltekt og sendingu eru íhlutir tínslur fyrir framleiðslu, samsetningu og vinnsluverkþætti á síðunni vöruhúsatínslu.
author: brentholtorf
ms.topic: conceptual
ms.devlang: na
ms.tgt_pltfrm: na
ms.workload: na
ms.search.keywords: ''
ms.date: 09/02/2022
ms.author: bholtorf
ms.openlocfilehash: 2ef879e5dbabb9281114d62a956ad4b10113c199
ms.sourcegitcommit: 8ad79e0ec6e625796af298f756a142624f514cf3
ms.translationtype: MT
ms.contentlocale: is-IS
ms.lasthandoff: 09/30/2022
ms.locfileid: "9607554"
---
# <a name="pick-for-production-assembly-or-jobs-in-advanced-warehouse-configurations"></a>Tínsla fyrir framleiðslu, samsetningu eða vinnslur í ítarlegum Vöruhúsafbrigðum

Í ítarlegum vöruhúsafbrigðum þar sem birgðageymslan er sett upp þannig að hún noti tiltekt og sendingu er hægt að velja íhluti fyrir framleiðslu-og samsetningarverkþætti á **síðunni vöruhúsatínslu**.  

Einnig er hægt að nota **síðuna Hreyfvinnublað** til að flytja vörur milli hólfa án tilvísunar í upprunaskjali. Frekari upplýsingar eru í [Færa vörur með ítarlegum vöruhúsaaðgerðum](warehouse-how-to-move-items-in-advanced-warehousing.md).  

Upplýsingar um tiltektarvörur í grunnvöruhúsastaðsetningum sem eru aðeins [settar upp fyrir tiltekt eru í færa íhluti í aðgerðasvæði í Grunnvöruvöruhúsi skilgreiningar](warehouse-how-to-move-components-to-an-operation-area-in-basic-warehousing.md).  

> [!NOTE]
> Ekki er hægt að stofna vöruhúsatínsluskjal frá grunni þar sem tínsluverkþáttur er alltaf hluti af verkflæði, annað hvort í Tognun-eða þrýstiaðstæðum.  

Einnig er hægt að stofna vöruhúsatínsluskjöl eins og ýtingu í glugganum **Stofna vöruhússtínslu** á upprunaskjalinu, eins og útgefna samsetningarpöntun eða vöruhúsaafhendingu. Frekari upplýsingar, sjá [Tína Vörur með vöruhúsatínslu](warehouse-how-to-pick-items-for-warehouse-shipment.md).  

Einnig er hægt að stofna vöruhúsatínsluskjal í togtísku með því að nota **síðuna tínsluvinnublað** til að greina tínslubeiðnir. Þessi aðferð hentar vel fyrir vörusendingar og innri rekstur. Síðan er hægt að stofna áskilin tínsluskjöl vöruhúss.  

Eftirfarandi ferli skýrir dæmi þar sem valdir eru íhlutir fyrir afhenta framleiðslupöntun á síðunni **Vinnublað tínslu**. Ferlið á einnig við fyrir samsetningarpöntun.  

Til að stofna tínslubeiðnir, bæði fyrir tog- og ýtidæmi, þurfa viðkomandi upprunaskjöl að vera útgefin. Upprunaskjöl eru gefin út fyrir innri aðgerðir á eftirfarandi hátt.  

|Upprunaskjal|Losunaraðferð|  
|---------------------|--------------------|  
|Framleiðslupöntun|Breyta tegund pöntunar í útgefna framleiðslupöntun.|  
|Samsetningarpöntun|Breyta stöðu í Útgefið.|
|Verk | Breyta stöðu í opna.|  

## <a name="to-pick-components-using-the-pick-worksheet"></a>Íhlutir tíndir úr tínsluvinnublöðunum

1. Veldu ![Ljósapera sem opnar eiginleika Viðmótsleitar.](media/ui-search/search_small.png "Segðu mér hvað þú vilt gera") táknið, fara í **Vinnublað tínslu** og velja síðan viðkomandi tengil.  
2. Velja skal **Sækja vöruhúsaskjöl** og síðan velja íhlutalínurnar í útgefnu framleiðslupöntuninni.  
3. Raða línum til að tryggja skilvirka tiltekt. Þú gætir viljað sameina línur til að spara tíma starfsmanna.  
4. Veldu aðgerðina **Stofna tínslu**.  
5. Skilgreina hvernig eigi að stofna vöruhúsatínsluskjöl og hvernig á að raða tínslulínunum með því að fylla út reiti á síðunni **Stofna tínslu**.  
6. Velja hnappinn **Í lagi**. Vöruhúsatínsluskjöl eru búin til með tínslulínum fyrir hvern íhlut sem er krafist í innri aðgerð.  

Aðgerðarsvæði, svo sem framleiðsluvinnugólf, gætu haft sjálfgefið hólf fyrir þá íhluti sem þeir krefjast. Ef svo er, er sjálfgefna hólfakóta bætt við skjalavöruhúsatínslu til að tilgreina hvar á að setja vörurnar. Frekari upplýsingar er að finna í verkfærunum fyrir **-framleiðsluhólfið** eða **reitina til-samsetningarkóti** hólfs.

## <a name="filling-the-consumption-bin"></a>Fylla út notkunarhólfið

Þetta flæðirit sýnir hvernig reiturinn **Hólfkóði** í framleiðslupöntunaríhlutalínum er útfylltur samkvæmt uppsetningu staðsetningar.

![Flæðirit hólfs.](media/binflow.png "BinFlow")  

## <a name="see-related-microsoft-training"></a>Sjá tengda [Microsoft-þjálfun](/training/paths/pick-ship-items-business-central/)

## <a name="see-also"></a>Sjá einnig .

[Vöruhúsastjórnun](warehouse-manage-warehouse.md)  
[Birgðir](inventory-manage-inventory.md)  
[Vöruhúsastjórnun sett upp](warehouse-setup-warehouse.md)  
[Samsetningardeild](assembly-assemble-items.md)  
[Hönnunarupplýsingar vöruhúsakerfi](design-details-warehouse-management.md)  
[Vinna með [!INCLUDE[prod_short](includes/prod_short.md)]](ui-work-product.md)


[!INCLUDE[footer-include](includes/footer-banner.md)]
