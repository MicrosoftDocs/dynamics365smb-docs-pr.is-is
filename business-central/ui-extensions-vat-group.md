...
2. Deilið tæknilegum upplýsingum sem meðlimir og fulltrúi VSK-hópsins þurfa á að halda til að tengjast [!INCLUDE[d365fin](includes/d365fin_md.md)]-leigjandanum sínum.

  Yfirleitt býr fulltrúi VSK-hópsins yfir þessum upplýsingum, t.d. heiti á umhverfinu sem fulltrúi VSK-hópsins notar til að taka á móti VSK frá meðlimum VSK-hópsins.
3. Búið til notendur í VSK-hóp [!INCLUDE[d365fin](includes/d365fin_md.md)] fulltrúans sem meðlimir hópsins geta notað til að auðkennt og tengt sig.
...
5. Setjið upp viðbótina fyrir umsjón VSK-hóps í VSK-hóp [!INCLUDE[d365fin](includes/d365fin_md.md)] fulltrúans með því að nota uppsetningarleiðbeininguna **Setja upp umsjón VSK-hópa** með hjálp. 

> [!NOTE]
> Til að tengjast fulltrúa VSK-hóps þurfa meðlimir notanda sem er með aðgang að VSK-hópnum í [!INCLUDE[d365fin](includes/d365fin_md.md)] fulltrúans. Fulltrúi VSK-hópsins verður að búa til að minnsta kosti einn notanda fyrir þetta, en af öryggisástæðum mælum við með því að notandi verði búinn til fyrir hvern meðlim VSK-hópsins. Gangið úr skugga um að dreifa innskráningarupplýsingunum til þessara meðlima VSK-hópsins á öruggan hátt.

## <a name="understanding-the-vat-group-management-setup"></a>Að skilja uppsetningu á umsjón VSK-hóps

Fulltrúi VSK-hópsins gefur upp API sem meðlimir hópsins geta notað til að tengjast við [!INCLUDE[d365fin](includes/d365fin_md.md)]-leigjandann sinn og senda VSK-skýrslur. Meðlimir VSK-hóps nota oft [!INCLUDE[d365fin](includes/d365fin_md.md)] í aðskildum Azure Active Directory-leigjendum. Þess vegna þarf fleiri uppsetningar til að koma á tengingu milli meðlima VSK-hópsins og [!INCLUDE[d365fin](includes/d365fin_md.md)] fulltrúans. 
...
Sannvottun er gerð af Azure Active Directory, þannig að uppsetningin verður að eiga sér stað í VSK-hópnum í Azure Active Directory fulltrúans til að leyfa tengingar. Skilgreina þarf skráningu Azure Active Directory-forrits með aðgang VSK-hópnum í [!INCLUDE[d365fin](includes/d365fin_md.md)] fulltrúans. Þetta gildir einnig ef fulltrúi VSK-hópsins notar [!INCLUDE[d365fin](includes/d365fin_md.md)] á staðnum. Þar að auki verður að skilgreina staka innskráningu ef fulltrúi VSK-hópsins er að nota [!INCLUDE[d365fin](includes/d365fin_md.md)] á staðnum.
...
Þegar fulltrúi VSK-hópsins notar [!INCLUDE[d365fin](includes/d365fin_md.md)] á netinu eða á staðnum, nota meðlimir VSK-hópsins Azure Active Directory til að sannvotta sig þegar þeir senda VSK-skýrslur til fulltrúa hópsins. Ef meðlimir VSK-hópsins eru einnig að nota [!INCLUDE[d365fin](includes/d365fin_md.md)] á netinu, getur meðlimur hópsins sannvottað sig með því að nota úthlutaðar innskráningarupplýsingar og upplýsingar um endastöð. Frekari upplýsingar er að finna í [Uppsetning á meðlimi VSK-hóps](ui-extensions-vat-group.md#vat-group-member-setup).

Meðlimir VSK-hóps sem eru með [!INCLUDE[d365fin](includes/d365fin_md.md)] á staðnum verða að setja upp skráningu forrits í Azure Active Directory fyrir VSK-hópinn í [!INCLUDE[d365fin](includes/d365fin_md.md)]-leigjanda fulltrúans. Skráning forritsins gerir [!INCLUDE[d365fin](includes/d365fin_md.md)] á netinu fyrir fulltrúa VSK-hópsins kleift að sannvotta meðlim hópsins. Frekari upplýsingar er að finna í [Stuttar leiðbeiningar: Skrá forrit með auðkenningarverkvangi Microsoft](/azure/active-directory/develop/quickstart-register-app).
...
* Í hlutanum **API-heimildir** skal bæta heimildum við [!INCLUDE[d365fin](includes/d365fin_md.md)]. Virkja úthlutun aðgangs fyrir **Financials.ReadWrite.All** og **user_impersonation** .
...
> [!NOTE]
> Áður en hafist er handa skal hafa samband við fulltrúa VSK-hópsins vegna eftirfarandi upplýsinga um [!INCLUDE[d365fin](includes/d365fin_md.md)]-leigjandann hans:
>
> * API-vefslóðin
> * Heiti fyrirtækisins 
> * Biðlarakenni
...
1. Veljið táknið ![Ljósaperuna sem opnar eiginleika Viðmótsleitar](media/ui-search/search_small.png "Segðu mér hvað þú vilt gera"), sláið inn **VSK-hóp** og veljið síðan tengda tengilinn.
2. Til að skilgreina hlutverk VSK-hóps fyrirtækisins skal velja **Meðlim** og síðan **Áfram** .
...
4. Bætið við **API-vefslóð** frá fulltrúa VSK-hópsins. Yfirleitt er vefslóðin á eftirfarandi sniði: `https://api.businesscentral.dynamics.com/v2.0/[TENANT-ID]/[ENVIRONMENTNAME]`. Til dæmis, `https://api.businesscentral.dynamics.com/v2.0/907869c3-b252-4aca-b9cb-17a15d25477b/UKRepresentative`. 
5. Bætið við [!INCLUDE[d365fin](includes/d365fin_md.md)] fyrirtækisheiti fyrir fulltrúa VSK-hópsins, t.d. *CRONUS UK Ltd.* .
6. Veljið **Gerð sannvottunar** , veljið **OAuth2** og síðan **Áfram** .
7. Í reitinn **Biðlarakenni** skal færa inn auðkennið sem fulltrúi VSK-hópsins bjó til.
8. Í reitinn **Leynilykill biðlara sem fulltrúi VSK-hópsins bjó til** skal færa inn leynilykilinn sem fulltrúi VSK-hópsins bjó til.
9. Í reitinn **OAuth 2.0 endastöð eftirlits** skal færa inn *https://login.microsoftonline.com/common/oauth2* .
10. Í reitinn **OAuth 2.0 vefslóð tilfangs** skal færa inn *https://api.businesscentral.dynamics.com/* .
11. Í reitinn **OAuth 2.0 framsendingarvefslóð** skal færa inn *https://businesscentral.dynamics.com/OAuthLanding.htm* . 
12. Þegar búið er að tilgreina ýmsa reiti skal velja **Áfram** og síðan færa inn innskráningarupplýsingar notanda sem fulltrúi VSK-hópsins bjó til.
13. Veljið skilgreiningu VSK-skýrslu sem er notuð til að skila inn VSK til yfirvalda í þínu landi.
