---
title: Framlenging VAT Group Management fyrir Bretland
...
1. Búðu til uppsetninguna í [Microsoft Entra ID uppsetningu fyrir hópmeðlimi](#microsoft-entra-id-setup-for-group-members).
...
3. Búðu til notendur sem meðlimir virðisaukaskattshópsins nota til að auðkenna þegar þeir tengjast fulltrúa virðisaukaskattshópsins [!INCLUDE[prod_short](includes/prod_short.md)]. Notendurnir verða að hafa fullgilt notendaleyfi fyrir [!INCLUDE[prod_short](includes/prod_short.md)].
...
Meðlimir virðisaukaskattshóps sem eru [!INCLUDE[prod_short](includes/prod_short.md)] á staðnum verða að setja upp appskráningu í Microsoft Entra auðkenni fyrir [!INCLUDE[prod_short](includes/prod_short.md)] leiganda virðisaukaskattshópsfulltrúa. Skráning forritsins gerir fulltrúa VSK-hópsins [!INCLUDE[prod_short](includes/prod_short.md)] netinu kleift að sannvotta meðlim hópsins. Frekari upplýsingar er að finna hér [Stuttar leiðbeiningar: Skráið forrit með auðkenningarverkvangi Microsoft](/azure/active-directory/develop/quickstart-register-app).

Þegar umsjónarmaður virðisaukaskattshópsmeðlims býr til appskráninguna í Microsoft Entra auðkenni verður hann að tilgreina eftirfarandi upplýsingar.
...
* Í **Auðkenning** hlutanum, í valkostinum til að velja **Styddar reikningsgerðir**, veljið **Reikningar í hvaða skipulagsskrá sem er (Hvaða Microsoft Entra skrá - Multitenant)**.
* Í hlutanum **Vottorð og leynilyklar** skal búa til nýjan leynilykil biðlara og skrá niður gildið. Meðlimir virðisaukaskattshópsins þurfa leyndarmálið þegar þeir setja upp tenginguna við hópfulltrúann.
...
3. Í reitnum **Biðlaraleyndarmál** tilgreinirðu leyndarmál viðskiptavinar frá appskráningu í Microsoft Entra ID.
...
4. Í reitnum **Reikningur fyrir hópuppgjör** skal tilgreina þann uppgjörsreikning sem notaður er fyrir VSK-fjárhæðir hópmeðlima. Þessi reikningur ætti að hafa **Eignir** sem **reikningsflokkur**.
5. Í reitnum **VSK-uppgjörsreikningur** skaltu tilgreina reikninginn sem þú notar fyrir VSK-uppgjör. Þessi reikningur ætti að hafa **skuldir** sem **reikningsflokkur**.
...
Þegar fulltrúi hópsins hefur skilað VSK-framtali til yfirvalda keyrir fulltrúinn venjulega aðgerðina **Reikna og bóka VSK-uppgjör**. Þessi aðgerð lokar opnum VSK-færslum og millifærir upphæðir á reikning VSK-uppgjörs. Sem stendur tekur þessi aðgerð ekki innsendingar hópsins með í reikninginn. Einungis VSK-færslur í fyrirtækinu í forsvari VSK-hópsins eru birtar. Upphæðir innsendinga frá meðlimum VSK-hópsins verða að vera bókaðar á VSK-uppgjörsupphæðina handvirkt þannig að VSK-uppgjörsreikningur hjá fulltrúa VSK-hópsins endurspegli skuldina sem tilkynnt var til yfirvalda. Þessi hegðun breytist í væntanlegri uppfærslu á [!INCLUDE[prod_short](includes/prod_short.md)], þannig að allur virðisaukaskattur hópsins (heildarupphæð á skýrslulínum virðisaukaskattsskýrslu) er gerður upp.
...
1. Skráðu þig inn á **Azure Portal** sem Authentication Administrator.  
2. Farðu í **Microsoft Entra ID**.   
3. Flettu til **Notendur** og veldu síðan notandann sem þú vilt framkvæma aðgerð.  
4. Veldu **Auðkenningaraðferðir** og efst á síðunni skaltu velja **Krefjast endurskráningar fjölþátta auðkenningar**. 
5. Farðu aftur í Dynamics 365 Business Central og veldu að endurnýja táknið úr **VSK Report Uppsetning**.  
