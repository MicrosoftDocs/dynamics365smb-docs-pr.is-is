...
* Herferðir þegar Sala uppfyllir skilyrðin fyrir sérstöku tilboði. Til dæmis gætu skilyrðin verið þegar pöntun uppfyllir lágmarksmagn, er á undan ákveðinni dagsetningu eða hefur ákveðna tegund af vöru.  
...
Eftir að búið er að setja upp sérstakt verð, [!INCLUDE[prod_short](includes/prod_short.md)] er hægt að reikna besta verð í sölu-og innkaupaskjölum, og í vinnslu-og birgðabókarlínum. Frekari upplýsingar í [besta verðútreikningi](sales-how-record-sales-price-discount-payment-agreements.md#best-price-calculation).
...
Sjálfgefið er að staða nýrra verðlista sé **Drög**. Drög að verðlista eru ekki innifalin í verðútreikningum. Þegar þú ert búin (NUM) að bæta við línum og vilt byrja að nota verðin skaltu breyta stöðunni í **virk**.
...
## <a name="using-sales-and-purchase-price-lists"></a>Notkun verðlista um sölu og innkaupsverð

> [!NOTE]
> Til að nota verðlista þarf stjórnandinn að hafa kveikt á eiginleikauppfærslunni **Ný söluverðsupplifun** í **Eiginleikastjórnun**. Læra meira at [gera komandi aðgerðir undan tíma](/dynamics365/business-central/dev-itpro/administration/feature-management) í stjórnun efni.
...
### <a name="converting-existing-prices-when-you-turn-on-the-pricing-feature-update"></a>Fyrirliggjandi verði breytt þegar kveikt er á uppfærslu á verðaðgerðinni
...
> [!NOTE]
> Ef verð eru aðeins stillt á vöru eða forðaspjöldum verða Sjálfgefnir verðlistar ekki fylltir út með þessum verðum við uppfærslu gagna. Hins vegar er hægt að opna einhvern af sjálfgefnum verðlistum eða **síðunni verð-vinnublað** og nota **aðgerðina leggja til línur** til að bæta við verðum sem stilltar eru á vöru eða forðaspjöldum.
...
1. **Á síðunni verð-vinnublað** skal velja **aðgerðina leggja til línur**.
2. **Í Verðlínunum-stofna nýja** síðu eru reitirnir fylltir út eins og þörf krefur. [!INCLUDE[tooltip-inline-tip](includes/tooltip-inline-tip_md.md)]
3. **Í reitnum Vöruafmörkun** eru afmarkanir tilgreindar fyrir valda **vörutegund**.
4. Reiturinn sjálfgildi **er** valinn til að tilgreina stillingar eins og:
   * Hvaða einingum verðlistinn fær úthlutað.
   * Dagsetningar þegar verðið er í gildi.
   * Gjaldmiðilskóði.
   * Afmörkunargerðin sem skilgreinir dálkana sem birtast í línuverðalistanum.
5. Velja **Í lagi**. Nýjum línum verður bætt **við síðuna verðblað** með völdum stillingum og einingarverðinu frá birgðaspjöldin.
6. Breyta stofnaða línunum með nýja einingarverðið eða afsláttunum. [!INCLUDE[tooltip-inline-tip](includes/tooltip-inline-tip_md.md)]

#### <a name="create-sales-price-lines-based-on-existing-price-lists"></a>Stofna Söluverð línur byggðar á fyrirliggjandi verðlistum

1. **Á síðunni verð-vinnublað** skal velja **aðgerðina afrita línur**.
2. **Á Verðlínunum-afrita** síðu sem fyrir er, veljið eldri verðlista í **svæðinu úr-verðlista**.
3. **Í reitnum Verðlínuafmörkun eru afmarkanir tilgreindar á afurðavalinnar** Verðskrá.
4. Reiturinn sjálfgildi **er** valinn til að tilgreina stillingar eins og:
   * Hvaða einingum verðlistinn fær úthlutað.
   * Dagsetningar þegar verðið er í gildi.
   * Gjaldmiðilskóði.
   * Afmörkunargerðin sem skilgreinir dálkana sem birtast í línuverðalistanum.
5. Aðrir reitir eru fylltir út eftir þörfum. [!INCLUDE[tooltip-inline-tip](includes/tooltip-inline-tip_md.md)]
6. Velja **Í lagi**. Nýjum línum verður bætt **við síðuna Verðblað** með völdum stillingum.
7. Breyta stofnaða línunum með nýja einingarverðið eða afsláttunum. [!INCLUDE[tooltip-inline-tip](includes/tooltip-inline-tip_md.md)]
...
5. Ef óskað er eftir að keyrslan aðlagist vöruverð sjálfkrafa skal færa inn leiðréttinguna í **reitinn Leiðréttingarstuðull**. Sem dæmi má nefna að hér er 1,15 í **leiðréttingarstuðli** fyrir 15% hækkun á vöruverði.  
...
>
> Til að setja upp verð eða línuafslætti fyrir alla viðskiptamenn, verðflokk viðskiptamanns eða herferð verður að opna síðurnar af birgðaspjaldi. Einnig er hægt að nota síðuna **Vinnublað söluverðs** fyrir söluverð. Frekari upplýsingar í [til að Fjöldauppfæra vöruverð](sales-how-record-sales-price-discount-payment-agreements.md#to-bulk-update-item-prices).  
...
Ný Uppsetning skilmála sölureikningsafsláttar er sett upp.

1. Á síðunni **Viðskiptamenn** skal velja aðgerðina **Reikningsafslættir**. Síðan **Reikningsafsláttur viðskm.** opnast.
2. **Í reitinn Gjaldmiðilskóti** er ritaður kóti fyrir gjaldmiðil sem skilmálar reikningsafsláttar í línunni eiga við. Reiturinn er skilinn eftir auður ef setja á upp reikningsafsláttarskilyrði í ISK.
