...
description: Lærðu að setja upp Intrastat-skýrsluaðgerðir og hvernig á að tilkynna viðskipti við fyrirtæki í öðrum ESB-löndum/-svæðum.
author: brentholtorf
ms.topic: conceptual
ms.devlang: na
ms.tgt_pltfrm: na
...
* **Upprunaland**: notað er tveggja stafliða ISO alfa kóðar fyrir land/svæði þar sem heppilegt var að fá eða framleiða. Ef góðkunni var framleitt í fleiri en einu landi/svæði er landsvæðið og upprunaland um það leyti Síðasta landið þar sem það var verulega unnið.
* **VSK-númer samstarfsaðila í aðildarríkinu þar sem innflutningur fer fram**: Þetta er VSK-númer samstarfsaðila í aðildarríkinu þar sem innflutningur fer fram. VSK-ID er einnig notað í skiptum innan ESB-útflutningsgagna meðal aðildarríkja og gerir aðildarríkjum kleift að úthluta mótteknum gögnum til innflutningsfyrirtækisins í eigin landi/á svæðinu. Tilkynningarskyldar vörur verða að tilgreina VSK-númer fyrirtækis sem taldi fram til tolls vegna vörukaupa í aðildarríkinu þar sem innflutningur fer fram.
...
* **Innsláttar-/brottfararstaðir** : Notið þessa til að tilgreina staðsetningar þar sem vörur eru sendar eða afhentar til eða frá öðrum löndum/svæðum. Heathrow flugstöðin er dæmi um komu-brottfararstað. Komu/brottfararstaði er hægt að færa inn í sölu- og innkaupskjöl á flýtiflipanum **Erlend viðskipti**. Þessar upplýsingar verða einnig afritaðar úr birgðafærslum þegar Intrastatbók er stofnuð.  
...
Í mörgum löndum/svæðum koma tolla-og skattayfirvöld í 8 stafa vörukóða fyrir ýmsar vörur. Ef birgðafærslur eiga að fela í sér nauðsynlegar upplýsingar þegar kerfið flytur þær í intrastatbókarlínu verða upplýsingar um tollflokk að hafa verið færðar inn á síðun **Tollflokkar**. Finna skal kóða þeirra vörutegunda sem fyrirtæki notanda verslar með og færa þá inn í gluggann **Tollflokkar**.
