---
title: Uppsetning pósthóps
description: Lærðu hvernig á að nota færsluhópa til að spara tíma og forðast mistök þegar þú bókar færslur.
author: brentholtorf
ms.author: bholtorf
...
---
# <a name="set-up-posting-groups"></a>Settu upp pósthópa
...
| Gerð | Lýsing |
| --- | --- |
| Almennir viðskiptabókunarflokkar |Úthlutaðu þessum hópi á viðskiptavini og lánardrottna til að tilgreina hverjum þú selur og hverjum þú kaupir frá. Settu upp þessa bókunarflokka á síðunni **Alm. viðskiptabókunarflokkar**. Þegar þú gerir það skaltu hugsa um hversu marga hópa þú þarft til að sundurliða sölu og innkaup. Til dæmis geturðu flokkað viðskiptavini og lánardrottna eftir svæði, eða eftir tegund viðskipta. |
| Almennir vörubókunarflokkar |Úthlutaðu þessum hópi á hluti og tilföng til að tilgreina hvað þú selur og hvað þú kaupir. Settu upp þessa bókunarflokka á síðunni **Alm. vörubókunarflokkar**. Þegar þú gerir það skaltu íhuga fjölda hópa sem þú þarft til að sundurliða sölu eftir vörum (hlutum og tilföngum) og innkaupum eftir vörum. Til dæmis, skaltu skipta þessum hópum eftir hráefnum, smásölu, tilföngum, getu og svo framvegis. |
...
| Birgðabókunarflokkur |Skilgreindu birgðabókunarflokka sem þú úthlutar síðan á viðeigandi vörureikninga á síðunni **Uppsetning birgðabókunar**. Þannig, þegar þú bókar færslur sem varða vöru, bókar kerfið á bókhaldsreikninginn sem er settur upp fyrir samsetningu birgðabókunarhóps og staðsetningar sem er tengdur við vöruna. Birgðabókunarflokkar bjóða einnig upp á góða leið til að skipuleggja birgðir þínar, þannig að þú getur aðskilið hluti eftir bókunarflokkum þegar þú býrð til skýrslur. Settu upp þessa bókunarflokka á síðunni **Birgðabókunarflokkar**. |
...
### <a name="allow-substitute-customer-or-vendor-posting-groups-on-documents"></a>Leyfa staðgengils viðskiptavina eða lánardrottna bókaflokka á skjölum

Þú getur látið fólk velja aðra bókunarhópa viðskiptavina og lánardrottna en sjálfgefna hópa þegar þeir eru að vinna með sölu- eða innkaupaskjöl og færslubækur.
...
| Uppsetning skattbókunar |Sameina viðskiptabókunarflokk skatta og vörubókunarflokk skatta. Þegar þú fyllir út almenna færslubókarlínu, innkaupalínu eða sölulínu, skoðum við samsetninguna til að auðkenna reikninga sem á að nota. |

Ef landið/svæðið þitt notar virðisaukaskatt (VSK), sjá [Setja upp útreikninga og bókunaraðferðir fyrir virðisaukaskatt](finance-setup-vat.md).  
...
## <a name="copy-posting-setup-lines"></a>Afritaðu uppsetningarlínur fyrir færslu

Því fleiri vöru- og fyrirtækjapóstahópa sem þú hefur, því fleiri línur muntu hafa á  **Almenn póstuppsetning** síðu. Þó að það gætu verið margar mismunandi samsetningar af viðskipta- og vörubókunarhópum, gætu mismunandi samsetningar samt bókað á sömu fjárhagslyklana. Til að draga úr handvirkum færslum skal afrita fjárhagsreikninga úr línu á síðunni **Alm. bókunargrunnur**.
...
Þannig, þegar þú vinnur að skjali sem notar bókunarhóp eða uppsetningu sem vantar áskilinn fjárhagsreikning, færðu tilkynningu. Veldu tengilinn í tilkynningunni til að opna síðu þar sem þú getur viðeigandi breytingar svo lengi sem þú hafir heimild til að gera slíkt.  
