---
title: "Um að setja upp Virðisaukaskatt | Microsoft Docs"
...
# <a name="setting-up-included365finlongincludesd365finlongmdmd-to-calculate-and-post-value-added-tax"></a>Uppsetning [!INCLUDE[d365fin_long](includes/d365fin_long_md.md)] til að reikna og bóka virðisaukaskatt
...
**Athugið**: Hægt er að nota leiðbeiningarnar, aðeins ef þú hefur stofnað Mitt fyrirtæki, og hefur ekki bókað færslur sem innihalda VSK. Annars er mjög auðvelt að nota mismunandi VSK taxta af misgáning og gera VSK-skýrslum ónákvæmt.  
...
* Setja upp VSK-vörubókunarflokka til að skilgreina VSK taxta fyrir vörur og þjónustu sem þú kaupir eða selur.  
   ...
   **Athugið**: Hugmyndirnar á bak við VSK-viðskipta- og vörubókunarflokka eru svipaðar og almennir bókunarflokkar. Nánari upplýsingar eru í [Uppsetning bókunarflokka](finance-posting-groups.md).
...
* Úthluta VSK-vörubókunarflokkum til fjárhagsreikningana sem þú notar fyrir sölu og innkaupum, og vörur, og tilföng.  
   ...
   **Athugið**: Til Að setja upp VSK fyrir tilföng, þarf að virkja **Pakki** notandaupplifunina fyrir þitt fyrirtæki. Nánari upplýsingar má sjá í [Sérstilli þína Dynamics 365 fyrir Financials upplifun](ui-experiences.md).
...
1. Efst í hægra horni skal velja **Leita að síðu eða skýrslu** táknið ![Leita að síðu eða skýrslu](media/ui-search/search_small.png "Leita að síðu eða skýrslu táknið"), færa **Uppsetning með hjálp**.  
...
1. Efst í hægra horni skal velja **Leita að síðu eða skýrslu** táknið ![Leita að síðu eða skýrslu](media/ui-search/search_small.png "Leita að síðu eða skýrslu táknið"), færa **VSK-viðskiptabókunarflokkur**, og velja síðan viðeigandi tengil.  
...
1. Efst í hægra horni skal velja **Leita að síðu eða skýrslu** táknið ![Leita að síðu eða skýrslu](media/ui-search/search_small.png "Leita að síðu eða skýrslu táknið"), færa **VSK-vörubókunarflokkar**, og velja síðan viðeigandi tengil.  
2. Fyllið inn í svæðin eftir þörfum.
...
1. Efst í hægra horni skal velja **Leita að síðu eða skýrslu** táknið ![Leita að síðu eða skýrslu](media/ui-search/search_small.png "Leita að síðu eða skýrslu táknið"), færa **VSK-bókunaruppsetning**, og velja síðan viðeigandi tengil.
2. Fyllið inn í svæðin eftir þörfum.
...
1. Efst í hægra horni skal velja **Leita að síðu eða skýrslu** táknið ![Leita að síðu eða skýrslu](media/ui-search/search_small.png "Leita að síðu eða skýrslu táknið") slá inn **Viðskiptamaður** eða **Lánardtottir** og velja svo tengdan tengil.  
...
1. Efst í hægra horni skal velja **Leita að síðu eða skýrslu** táknið ![Leita að síðu eða skýrslu](media/ui-search/search_small.png "Leita að síðu eða skýrslu táknið"), slá inn **Vara** eða **Forði** og velja svo tengdan tengil.  
2. Gert er eitt af eftirfarandi:
* Á **Vöru** spjaldinu, stækkið **Verð & Bókun** Flýtiflipann, og velja síðan **Sýna fleiri** til að birta .  
...
1. Efst í hægra horni skal velja **Leita að síðu eða skýrslu** táknið ![Leita að síðu eða skýrslu](media/ui-search/search_small.png "Leita að síðu eða skýrslu táknið"), færa **VSK ákvæði**, og velja síðan viðeigandi tengil.  
...
3. Efst í hægra horni skal velja **Leita að síðu eða skýrslu** táknið ![Leita að síðu eða skýrslu](media/ui-search/search_small.png "Leita að síðu eða skýrslu táknið"), færa **VSK-bókunaruppsetning**, og velja síðan viðeigandi tengil.  
...
Framkvæmdarstjórn Evrópusambandsins býður á vefsíðu sinni þjónustu VIES varðandi VSK Númerastaðfestingu, sem býðst öllum og er án endurgjalds. Þú ert samt sem áður einu skrefi frá því komast á vefsíðuna. [!INCLUDE[d365fin](includes/d365fin_md.md)] getur sparað þér það skref og leyft þér að nota þjónustu VIES til að staðfesta og rekja VSK-númer viðskiptamanna, lánardrottna og tengiliði beint úr spjöldum viðskiptamaður, lánardrottinn og tengiliða. Þjónustan í [!INCLUDE[d365fin](includes/d365fin_md.md)] kallast ESB VSK Reg. Númer Prófunarþjónusta. Er tiltækur í **Þjónustutengingar** síðu og þú getur hafið notkun með því aðeins að velja gátreit. Þjónustan er án endurgjalds og skráningar er ekki krafist.

Þegar þjónustu okkar er notuð, skráum við ferill VSK-númera og VSK-staðfestinga fyrir hvern viðskiptamann, lánadrottinn eða tengiliðar, í reitnum **VSK Skráning Skrá**, þannig að þú getur auðveldlega rekja þá. Skráin á sérstaklega við hvern viðskiptamann. Til dæmis kemur skráin að gagni við að sanna það að þú hafir staðfest að gildandi VSK-númer séu réttar. Þegar VSK-númer er staðfest, mun **Biðja um kenni** dálkurinn í skránni endurspegla að þú hafir framkvæmt hlutinn. 
...
**Athugasemdir**: Það eru nokkur atriði sem vert er að athuga varðandi þjónustu VIES á VSK-númerastaðfestingu:
...
**Athugið**: Þessi regla á við þegar skipt er við fyrirtæki sem eru skráð VSK-skyld í öðrum ESB-löndum/svæðum. Ef skipt er beint við viðskiptamenn í öðrum ESB-löndum/svæðum ætti að hafa samband við skattayfirvöld til að fá upplýsingar um viðeigandi VSK-reglur.  

**Ábending**: Þú getur staðfest að fyrirtæki sé skráð VSK-skylt í öðru ESB-landi með því að nota ESB staðfestingarþjónustu VSK-númera. Þjónustan er tiltæk án endurgjalds í [!INCLUDE[d365fin](includes/d365fin_md.md)]. Frekari upplýsingar má finna í hlutanum sem nefndur er _Staðfesting VSK-númera_ í þessu efnisatriði.
